...
JSDOC.Symbol.prototype.setTags = function() {
	...
	*/
	...
	if (classes.length) {
        ...
        // @enum
        if (this.comment.getTag("enum").length) {
            this.isEnum = true;
        }
	}
	...
	*/
    ...
    /* @this */
    var these = this.comment.getTag("this");
    if (these.length) {
        // For documenting the instance in which a fucntion/method is called
		this.altThis = these[0].desc;
	}
	...
	*/
    ...
    // @category
	var categories = this.comment.getTag("category");
	if (categories.length) {
		this.category = categories[0].desc;
	}
    
    // @abstract
	var abstracts = this.comment.getTag("abstract");
	if (abstracts.length) {
	...
	*/
	...
	if (properties.length) {
		...
		for (var i = 0; i < properties.length; i++) {
            ...
            if (properties[i].value) property.value = properties[i].value;
	...
	*/
    ...
    // @value
	var values = this.comment.getTag("value");
	if (values.length) {
		if (this.is("OBJECT")) {
			this.value = values[0].desc;
		}
	}
