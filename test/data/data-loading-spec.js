define(['cdf/lib/CCC/pvc-d1.0', 'test-utils', 'test-data-1'], function(pvc, utils, datas) {
    ...
    describe('Data loading', function() {
        ...
        describe('of cross-tab data:', function() {
            var data = utils.loadData(datas['cross-tab, category missing on first series']);

            it('should preserve source row-major order', function() {
                var datums = data.datums().array();

                expect(datums.length).toBe(6);
            ...
            it('should ignore source value order, in favor of numeric order, in numeric dimensions', function() {
                ...
                expect(atoms[2].value).toBe(50);
                expect(atoms[3].value).toBe(60);
                expect(atoms[4].value).toBe(72);
            });
        });
    });
