...
.add(/** @lends pvc.data.ComplexTypeProject# */{
    _ensureDim: function(name, spec) {
        ...
        var info = def.getOwn(this._dimensions, name);
        if(!info) {
            info = this._dimensions[name] = this._createDim(name, spec);
    ...
    hasDim: function(name) { return def.hasOwn(this._dimensions, name); },
